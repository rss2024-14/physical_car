...
class PathPlan(Node):
    ...
    def map_cb(self, map_1d):
        ...
        self.map_2d = dilate_array(self.map_2d)
...
def dilate_array(arr):
    ...
    for r in range(rows):
        for c in range(cols):
            if arr[r, c] == 100:
                row_start = max(0, r-10)
                row_end = min(rows, r+11)
                col_start = max(0, c-10)
                col_end = min(cols, c+11)

                output[row_start:row_end, col_start:col_end] = 100
